...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__libs_wetty_wetty__ = __webpack_require__(47);
//
//
//
//
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return hterm; });
...
lib.MessageManager.prototype.processI18nAttributes = function (dom) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    for (var key in i18n) {
      ...
      if (msgname.substr(0, 1) == '$') msgname = thunk(node.getAttribute(msgname.substr(1)) + '_' + key);
...
lib.TestManager.TestRun.prototype.onResultComplete = function (result) {
  ...
  if (result.status == result.FAILED) {
    ...
    this.log.println('Unknown result status: ' + result.test.fullName + ': ' + result.status);
...
lib.TestManager.TestRun.prototype.runNextTest_ = function () {
  ...
  try {
    ...
    this.log.println('Unexpected exception during test preamble: ' + (ex.stack ? ex.stack : ex));
    this.log.popPrefix();
    this.log.println('}');
  ...
  try {
    ...
    this.log.println('Unexpected exception during test run: ' + (ex.stack ? ex.stack : ex));
    this.panic = true;
  }
};
...
lib.TestManager.TestRun.prototype.summarize = function () {
  ...
  if (this.testQueue_.length) {
    this.log.println('Test run incomplete: ' + this.testQueue_.length + ' test(s) were not run.');
  }
};
...
lib.registerInit('hterm', function (onInit) {
  ...
  if (!hterm.defaultStorage) {
    ...
    if (window.chrome && chrome.storage && chrome.storage.sync && version > 21) {
...
hterm.Frame.prototype.onLoad_ = function () {
  ...
  this.iframe_.contentWindow.postMessage({ name: 'ipc-init', argv: [{ messagePort: this.messageChannel_.port2 }] }, [this.messageChannel_.port2], this.url);
};
...
hterm.Frame.prototype.show = function () {
  ...
  header.style.cssText = 'display: -webkit-flex;' + '-webkit-justify-content: flex-end;' + 'height: ' + headerHeight + ';' + 'background-color: ' + this.terminal_.getForegroundColor() + ';' + 'color: ' + this.terminal_.getBackgroundColor() + ';' + 'font-size: 16px;' + 'font-family: ' + this.terminal_.getFontFamily();
...
hterm.Keyboard.prototype.onKeyUp_ = function (e) {
  ...
  if (e.keyCode == 27) this.preventChromeAppNonShiftDefault_(e);
};
...
hterm.Keyboard.prototype.onKeyDown_ = function (e) {
  ...
  if (action === STRIP) {
    ...
    if (action == DEFAULT && keyDef.keyCap.length == 2) action = keyDef.keyCap.substr(e.shiftKey ? 1 : 0, 1);
  }
...
hterm.Keyboard.KeyMap.prototype.addKeyDefs = function (var_args) {
  for (var i = 0; i < arguments.length; i++) {
    this.addKeyDef(arguments[i][0], { keyCap: arguments[i][1],
...
hterm.Keyboard.KeyMap.prototype.reset = function () {
  ...
  // Firefox Italian +*
  [171, '+*', DEFAULT, c('onPlusMinusZero_'), DEFAULT, c('onPlusMinusZero_')], [8, '[BKSP]', bs('\x7f', '\b'), bs('\b', '\x7f'), DEFAULT, DEFAULT],
  ...
  [38, '[UP]', ac(CSI + 'A', SS3 + 'A'), DEFAULT, DEFAULT, DEFAULT], [40, '[DOWN]', ac(CSI + 'B', SS3 + 'B'), DEFAULT, DEFAULT, DEFAULT], [39, '[RIGHT]', ac(CSI + 'C', SS3 + 'C'), DEFAULT, DEFAULT, DEFAULT], [37, '[LEFT]', ac(CSI + 'D', SS3 + 'D'), DEFAULT, DEFAULT, DEFAULT], [144, '[NUMLOCK]', PASS, PASS, PASS, PASS],
  ...
  [96, '[KP0]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [97, '[KP1]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [98, '[KP2]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [99, '[KP3]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [100, '[KP4]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [101, '[KP5]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [102, '[KP6]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [103, '[KP7]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [104, '[KP8]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [105, '[KP9]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [107, '[KP+]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [109, '[KP-]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [106, '[KP*]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [111, '[KP/]', DEFAULT, DEFAULT, DEFAULT, DEFAULT], [110, '[KP.]', DEFAULT, DEFAULT, DEFAULT, DEFAULT],
...
hterm.Keyboard.KeyMap.prototype.onCtrlC_ = function (e, keyDef) {
  ...
  if (!selection.isCollapsed) {
    if (this.keyboard.ctrlCCopy && !e.shiftKey) {
      ...
      if (this.keyboard.terminal.prefs_.get('enable-clipboard-notice')) setTimeout(this.keyboard.terminal.showOverlay.bind(this.keyboard.terminal, hterm.notifyCopyMessage, 500), 200);
...
hterm.Screen.prototype.insertString = function (str) {
  ...
  if (reverseOffset < 0) {
    ...
    if (!(this.textAttributes.underline || this.textAttributes.strikethrough || this.textAttributes.background || this.textAttributes.wcNode || this.textAttributes.tileData != null)) {
      // Best case scenario, we can just pretend the spaces were part of the
      // original string.
      str = ws + str;
    } else if (cursorNode.nodeType == 3 || !(cursorNode.wcNode || cursorNode.tileNode || cursorNode.style.textDecoration || cursorNode.style.backgroundColor)) {
...
hterm.ScrollPort.prototype.decorate = function (div) {
  ...
  doc.body.style.cssText = 'margin: 0px;' + 'padding: 0px;' + 'height: 100%;' + 'width: 100%;' + 'overflow: hidden;' + '-webkit-user-select: none;' + '-moz-user-select: none;';
  ...
  this.screen_.style.cssText = 'display: block;' + 'font-family: monospace;' + 'font-size: 15px;' + 'height: 100%;' + 'overflow-y: scroll; overflow-x: hidden;' + 'white-space: pre;' + 'width: 100%;' + 'outline: none !important';
  ...
  this.topSelectBag_.style.cssText = 'display: block;' + 'overflow: hidden;' + 'white-space: pre;';
...
hterm.ScrollPort.prototype.syncCharacterSize = function () {
  ...
  if (this.DEBUG_) {
    ...
    this.document_.body.style.paddingTop = this.document_.body.style.paddingBottom = 3 * this.characterSize.height + 'px';
  }
};
...
hterm.ScrollPort.prototype.onCopy_ = function (e) {
  ...
  if (!this.selection.startRow || this.selection.endRow.rowIndex - this.selection.startRow.rowIndex < 2) {
    return;
  }
...
hterm.Terminal.prototype.runCommandClass = function (commandClass, argString) {
  ...
  var self = this;
  this.command = new commandClass({ argString: argString || '',
    io: this.io.push(),
...
hterm.Terminal.prototype.setFontSize = function (px) {
  ...
  if (this.wcCssRule_) {
    this.wcCssRule_.style.width = this.scrollPort_.characterSize.width * 2 + 'px';
  }
};
...
hterm.Terminal.prototype.decorate = function (div) {
  ...
  style.textContent = '.cursor-node[focus="false"] {' + '  box-sizing: border-box;' + '  background-color: transparent !important;' + '  border-width: 2px;' + '  border-style: solid;' + '}' + '.wc-node {' + '  display: inline-block;' + '  text-align: center;' + '  width: ' + this.scrollPort_.characterSize.width * 2 + 'px;' + '}';
  ...
  this.cursorNode_.style.cssText = 'position: absolute;' + 'top: -99px;' + 'display: block;' + 'width: ' + this.scrollPort_.characterSize.width + 'px;' + 'height: ' + this.scrollPort_.characterSize.height + 'px;' + '-webkit-transition: opacity, background-color 100ms linear;' + '-moz-transition: opacity, background-color 100ms linear;';
...
hterm.Terminal.prototype.syncCursorPosition_ = function () {
  ...
  this.cursorNode_.style.top = this.scrollPort_.visibleRowTopMargin + this.scrollPort_.characterSize.height * (cursorRowIndex - topRowIndex) + 'px';
...
hterm.Terminal.prototype.onMouse_ = function (e) {
  ...
  if (this.options_.cursorVisible && this.vt.mouseReport == this.vt.MOUSE_REPORT_DISABLED) {
  ...
  if (e.type == 'mouseup' && this.document_.getSelection().isCollapsed) {
    ...
    this.reportMouseEvents_ = this.vt.mouseReport != this.vt.MOUSE_REPORT_DISABLED;
  }
};
...
hterm.Terminal.prototype.onResize_ = function () {
  var columnCount = Math.floor(this.scrollPort_.getScreenWidth() / this.scrollPort_.characterSize.width);
  var rowCount = lib.f.smartFloorDivide(this.scrollPort_.getScreenHeight(), this.scrollPort_.characterSize.height);
...
hterm.TextAttributes.prototype.syncColors = function () {
  ...
  if (this.enableBoldAsBright && this.bold) {
    if (foregroundSource != this.SRC_DEFAULT && foregroundSource != this.SRC_RGB) {
  ...
  if (foregroundSource != this.SRC_RGB) {
    this.foreground = foregroundSource == this.SRC_DEFAULT ? defaultForeground : this.colorPalette[foregroundSource];
  }
  ...
  if (backgroundSource != this.SRC_RGB) {
    this.background = backgroundSource == this.SRC_DEFAULT ? defaultBackground : this.colorPalette[backgroundSource];
  }
};
...
hterm.VT.prototype.parseUntilStringTerminator_ = function (parseState) {
  ...
  if (nextTerminator == -1) {
    ...
    if (args[0].indexOf('\x1b') != -1) abortReason = 'embedded escape: ' + args[0].indexOf('\x1b');
  ...
  parseState.resetParseFunction();
  parseState.advance(nextTerminator + (buf.substr(nextTerminator, 1) == '\x1b' ? 2 : 1));
...
lib.resource.add('hterm/concat/date', 'text/plain', 'Fri, 03 Apr 2015 09:02:15 +0000' + '');

lib.resource.add('hterm/changelog/version', 'text/plain', '1.54' + '');

lib.resource.add('hterm/changelog/date', 'text/plain', '2015-03-19' + '');

lib.resource.add('hterm/git/HEAD', 'text/plain', '5db5b0ceefff97ce8b7a97edd3f5ba6857db54cf' + '');
window.lib = lib;
window.hterm = hterm;
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__hterm_all_js__ = __webpack_require__(45);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__socket_io_js__ = __webpack_require__(46);
